...
const dispatchWeb3Event = (status, address, balance, web3, error) => {
  document.dispatchEvent(new CustomEvent('web3-widget-event', { detail: { status, address, balance, network: config.network, web3, ens: ens || "", error, walletType: walletInfo?.walletType || '', disconnect: onDisconnect } }));
};
